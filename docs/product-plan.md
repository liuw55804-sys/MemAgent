...
对标调研见 [competitive-scan.md](competitive-scan.md)，v0.10 定位校准见 [design_v0.10_competitive_positioning.md](design_v0.10_competitive_positioning.md)，v0.24 GitHub landscape refresh 见 [design_v0.24_landscape_refresh.md](design_v0.24_landscape_refresh.md)。当前差异化重点不是“也做长期记忆”，而是 Codex-first 的 workflow memory lifecycle：围绕真实工程线程里的工具 recipe、失败路径、数据入口和验证方式做记录、召回、注入、评估和升格。
...
这些句子不是硬编码触发词，而是给 Codex / LLM router 读的语义示例。理想路径由 AI API 判断当前是不是“召回、沉淀、反馈、handoff、普通对话”等自然交互节点；本地 heuristic 只作为离线兜底和可测试 baseline：
...
这时 `AGENTS.md` 不是存储 memory 的地方，而是告诉 Codex：“看到这类语义时，可以把 MemAgent 当作后台记忆工具”。v0.26 先用 AGENTS.md prompt policy 校准行为；v0.27 将判断逻辑抽成 `memagent route` / `memagent_route`；v0.28 用 `draft memory` / `memagent_memory_draft` 做候选改写和质量判断；v0.29 用 `process` / `memagent_process` 把 route 和安全动作串成一个 Codex-native 入口；v0.30 用 `llm doctor` / `memagent_llm_doctor` 让 OpenAI-compatible provider 的配置和 live check 可诊断；v0.31 让 `memagent codex` wrapper 也走 process-first preflight，不再硬编码 recall-first。这些能力都用同一类 schema 支持本地 heuristic 和 OpenAI-compatible LLM provider。
