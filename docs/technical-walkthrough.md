...
PYTHONPATH=src python -m memagent.cli mcp-demo --reset
...
当前主要命令是 `remember`、`recall`、`route`、`draft`、`process`、`llm`、`trace`、`codex`、`agents-snippet`、`agents-install`、`agents-doctor`、`ingest`、`handoff`、`demo-run`、`demo-bundle`、`mcp-demo`、`mcp-stdio`、`recall-eval`。
...
- 正常路径下可以先调用 `memagent process`，让 MemAgent 自己 route 并执行安全动作。
- 用户想接 DeepSeek/Qwen/GLM/OpenAI-compatible API 时，可以先调用 `memagent llm doctor` 检查 provider readiness；多个 provider 可以放在 `~/.memagent/llm_providers.local.json`，并用 `--profile` 选择；只有显式要求时才用 `--check-live` 发真实请求。
...
它会创建 mock project 和本地 memory home，然后直接调用 `McpServer.handle(...)`，依次记录：
...
- `tools/call memagent_handoff_save/show`
- `tools/call memagent_trace_list/label/report/eval`
...
main(argv)
  ...
  -> if codex: process preflight + build_augmented_prompt + subprocess.run
